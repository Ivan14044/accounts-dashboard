 ...
 * а параллельность обеспечивает bulk API check.fb.tools на бэкенде.
 *
 * BATCH_SIZE=100 → один /check запрос проверяет 100 аккаунтов за ~1-2 сек.
 * UI обновляется плавно после каждого батча. Ошибки не крашат процесс.
 */
(function () {
  'use strict';

  // ─── Константы ─────────────────────────────────────────
  var BATCH_SIZE  = 100;   // элементов за один /check запрос
