...
(function() {
  ...
  function showImportResults(result) {
    ...
    if (skipped > 0) {
      html += '<div class="col-md-3"><div class="card border-info"><div class="card-body text-center">';
      html += '<i class="fas fa-copy fa-2x text-info mb-2"></i>';
      html += '<h3 class="mb-0">' + skipped + '</h3>';
      html += '<small class="text-muted">Дубликатов</small>';
      html += '</div></div></div>';
    }
  ...
  async function handleUpload(e) {
    ...
    // Режим «Обновить» перезаписывает данные существующих аккаунтов, а импорт НЕ
    // пишет в журнал отмены — откатить нечем. Поэтому спрашиваем явно, и только
    // для этого режима: «Пропустить» и «Показать ошибку» ничего не портят.
    if (formData.get('duplicate_action') === 'update') {
      const ok = window.confirm(
        'Режим «Обновить»: данные существующих аккаунтов будут перезаписаны значениями из файла.\n\n' +
        'Пустые ячейки не тронут поле, но заполненные заменят текущие значения.\n' +
        'Отменить импорт нельзя — он не попадает в журнал отмены.\n\nПродолжить?'
      );
      if (!ok) {
