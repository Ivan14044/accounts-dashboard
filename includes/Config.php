...
class Config {
    ...
    /**
     * TTL файлового кэша агрегатов дашборда, секунды.
     *
     * Держим маленьким: это страховка, а не основной механизм свежести. Основной —
     * отпечаток данных в ключе (MAX(updated_at)), из-за которого правка строки
     * инвалидирует кэш мгновенно. TTL закрывает только жёсткое удаление строк.
     */
    const STATS_FILE_CACHE_TTL = 30;

    /**
     * Ширина «корзины» отпечатка данных, секунды.
     *
     * Отпечаток агрегатов — MAX(updated_at) по таблице. В точном виде он делал
     * кэш бесполезным на живой панели: любая правка ЛЮБОЙ строки кем угодно
     * меняла ключ, и следующий открывший страницу платил полный пересчёт
     * (замер на 184 800 строках: 0,58 с против 0,045 с из кэша). А правки идут
     * постоянно, поэтому в пересчёт попадали почти все заходы.
     *
     * Теперь отпечаток округляется вниз до этой границы: правки, случившиеся
     * в пределах одного окна, делят общий ключ. Цена — счётчики на карточках
     * могут отставать от чужих правок не больше чем на это время. Свои правки
     * видно сразу: тот, кто только что писал в таблицу, до конца окна получает
     * ТОЧНЫЙ отпечаток (см. STATS_SELF_WRITE_FLAG и Database::prepare()).
     */
    const STATS_FINGERPRINT_BUCKET = 30;

    /**
     * Ключ в сессии: до какого момента этот пользователь считается «только что
     * писавшим» и должен видеть точные, не округлённые счётчики.
     */
    const STATS_SELF_WRITE_FLAG = 'stats_self_write_until';
     ...
     *
     * 1000 (а не 100) — подтверждено живыми замерами 2026-06: API принимает
     * крупные партии (1000 → ~0.5с/112KB, 3000 → ~4.7с, 5000 → ~8с, все HTTP 200)
     * и НЕ троттлит серию запросов. Крупный батч ⇒ в ~10 раз меньше HTTP-запросов
     * ⇒ меньше шанс упереться в rate-limit нашего nginx и быстрее общий прогон.
     * 1000 выбран как баланс: суб-секундный ответ и умеренный объём для json_decode.
     */
    const FB_TOOLS_BATCH_SIZE = 1000;

    /**
     * Таймаут запроса к check.fb.tools (секунд).
     * 30 сек — запас под крупные батчи (1000 ID ≈ 0.5с, но даём маржу на
     * сетевые задержки/пиковую нагрузку), при сбое уходим в retry.
     */
    const FB_TOOLS_TIMEOUT = 30;
    ...
    /**
     * Максимум записей (аккаунтов) за один запрос validate/check.
     *
     * 1000 (вместо 200) — синхронно с FB_TOOLS_BATCH_SIZE=1000: фронт шлёт по
     * 1000 аккаунтов за заход (CONCURRENCY=1), сервер укладывает их обычно в
     * 1-2 параллельных запроса к check.fb.tools. Так на 3000 аккаунтов уходит
     * ~3 round-trip'а вместо ~15, а данные-портящие сбои (errored) безопасны
     * благодаря classifyItems(). set_time_limit в роуте (120с) перекрывает это.
     */
    const VALIDATE_CHECK_MAX_ITEMS = 1000;
