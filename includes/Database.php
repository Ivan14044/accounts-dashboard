...
class Database {
    ...
    private function __construct() {
        ...
        // range_optimizer_max_mem_size — память, которую оптимизатор тратит на разбор
        // диапазонов. Если длинный список «id IN (...)» в неё не помещается, MySQL
        // МОЛЧА отказывается от плана по первичному ключу и читает таблицу целиком.
        //
        // Именно это и происходило на проде: в медленном журнале 42 подряд идущих
        // массовых смены статуса (1000 id на запрос) читали ровно 185 037 строк —
        // всю таблицу — и шли по 66–68 секунд каждая; удаление в корзину так же,
        // 193 001 строка и до 67 секунд. Воспроизведено на стенде (mysql:5.7,
        // 185 000 строк прод-формы, настоящий prepared statement из PHP):
        //   лимит 16 КБ: 1000 id → 5,01 с, Handler_read_rnd_next = 185 001 (полный скан)
        //   лимит 8 МБ:  1000 id → 0,93 с, Handler_read_rnd_next = 0 (поиск по PK)
        // Дробить пачку бесполезно: при лимите 16 КБ даже 200 id читали всё.
        //
        // Поэтому поднимаем лимит своей сессии до дефолта MySQL (8 МБ). GREATEST —
        // чтобы не опустить лимит там, где сервер настроен щедрее; отдельная ветка
        // для нуля — потому что 0 означает «без лимита» и его портить нельзя.
        // На MariaDB и MySQL < 5.7.9 переменной нет, запрос вернёт false — это
        // безопасно: поведение остаётся прежним. (проверено 2026-09-03)
        $this->mysqli->query(
