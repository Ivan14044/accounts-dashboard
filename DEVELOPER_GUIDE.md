...
- Бандлов три: `core.css` (весь свой CSS, общий для дашборда, избранного и
  ...
  (10 скриптов с `defer`). Разделение sync/defer обязательное: `defer`-скрипты
...
### Телефон и планшет: мобильный слой

Всё «телефонное» живёт в двух файлах, общих для ВСЕХ страниц панели:

- `assets/css/core-touch.css` — подключается последним: на дашборде, в
  избранном и корзине — последним файлом бандла `core.css`, на остальных
  страницах — отдельным `<link>` сразу перед `</head>`, после `<style>`
  страницы. Правила про палец (цели 44×44, шрифт полей 16px, реакция на
  нажатие) — только под `@media (pointer: coarse)`; правила раскладки (окна-
  шторки снизу, нижняя панель вкладок, панель выбранных строк, закреплённый
  номер в таблице) — только под `max-width ≤ 767/575px`. Поэтому мышь на
  мониторе этих правил не видит ни на какой ширине.
- `assets/js/mobile-touch.js` — жест «смахнуть окно вниз», нажатие по всей
  ячейке с галочкой (кроме главной таблицы, где строка и так отмечается по
  щелчку) и класс `body.kb-open`, пока открыта экранная клавиатура.

Что нужно помнить при правках:

- Новая страница с целым HTML-документом: `viewport-fit=cover` в meta
  viewport и `<link>` на `core-touch.css` перед `</head>`. Есть окна — ещё
  `<script src="assets/js/mobile-touch.js" defer>`.
- Высота «на весь экран» — `100vh` и сразу строкой ниже `100dvh`.
- Числовое поле — `type="number" inputmode="numeric"`.
- Панель действий с выбранными строками на телефоне появляется по классу
  `body.has-selection`; его ставят `dashboard-selection.js` и `trash.js`.
- Инварианты стережёт `tests/test_mobile_touch_layer.php`, в том числе «ни
  одного 44px вне медиа-условия» — такое правило изменило бы вид на компьютере.
