...
check($p['trashed_included'] === 1, 'сколько корзинных вошло в откат — видно отдельно');
...
echo "── Режим «вернуть всех» (forceAll): поздние правки СОЗНАТЕЛЬНО перезаписываются\n";
// Инцидент 03.09.2026: аккаунты после операции успела отработать внешняя
// программа, в «своём» статусе не осталось ни одного. Владелец решил вернуть
// все 333 — это его явное решение, поэтому режим включается отдельным флагом.
$p = StatusRevertPlanner::plan([row(1, CUR), row(2, 'perechek_new_x'), row(3, null), row(4, '')], CUR, false, true);
check($p['revert'] === [1, 2, 3, 4], 'forceAll: откатываются и изменённые, и NULL/пустые');
check($p['skipped_changed'] === 0, 'forceAll: пропусков по «изменён» нет');
check($p['forced_changed'] === 3, 'forceAll: сколько чужих правок перезаписано — посчитано отдельно');

echo "── forceAll не отменяет правило про корзину\n";
$p = StatusRevertPlanner::plan([row(1, 'sale_2', '2026-09-06 08:11:44'), row(2, 'sale_2')], CUR, false, true);
check($p['revert'] === [2], 'forceAll без includeTrashed: корзина всё равно пропущена');
check($p['skipped_trashed'] === 1, 'forceAll: пропуск по корзине посчитан');
$p = StatusRevertPlanner::plan([row(1, 'sale_2', '2026-09-06 08:11:44'), row(2, 'sale_2')], CUR, true, true);
check($p['revert'] === [1, 2], 'forceAll + includeTrashed: возвращаются все');
check($p['trashed_included'] === 1 && $p['skipped_trashed'] === 0, 'forceAll + includeTrashed: корзинный посчитан как вошедший');

echo "── Без forceAll поведение прежнее (forced_changed = 0)\n";
$p = StatusRevertPlanner::plan([row(1, CUR), row(2, 'sale_2')], CUR, false);
check($p['revert'] === [1] && $p['forced_changed'] === 0, 'по умолчанию чужие правки не затираются');
