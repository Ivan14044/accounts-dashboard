...
- **Поиск (addSearchFilter)** — поиск двухфазный, и стоимость у фаз разная.
  - *Текстовый запрос* — сразу `login LIKE '%...%' OR email LIKE '%...%' OR social_url LIKE '%...%' OR first_name LIKE '%...%' OR last_name LIKE '%...%'`. Префиксные индексы для `%...%` не помогают; возможен полный скан по отфильтрованной части таблицы. Это ожидаемо для «поиска по подстроке»; снизить стоимость можно только ограничением других условий (хорошие индексы по deleted_at, status и т.д.) и лимитами.
  - *Числовой запрос или ссылка с ID* — фаза 1 сравнивает точно: `id = ? OR login = ? OR id_soc_account = ? OR id_fan_page_1..3 = ?`. Все поля индексированы (`id` — PRIMARY), полного скана нет. `id` попадает в условие только для правдоподобного значения (без ведущего нуля, ≤ 4294967295): сравнение INT-колонки со строкой MySQL делает приведением, и `id = '007'` нашло бы чужую строку с id 7, а условие по числу длиннее потолка типа не нашло бы ничего (проверено на mysql:8.0).
  - Фаза 2 включается вызовом `fallbackToLikeSearch()` только если фаза 1 вернула 0 строк (см. `DashboardController` и `refresh.php`) и меняет условие на LIKE по login/email/social_url/id_fan_page_*/cookies/first_cookie/token — это дорогой полный скан, зато находит FB ID внутри JSON cookies.
